      ...
      ORDER BY 
        ...
        const ordersWithItems = await Promise.all(
          orders.map(async (order) => {
              // First query - get items with product names
              const [itemsWithProductNames] = await pool.query(
              ...
              // Second query - get all order items
              const [simpleItems] = await pool.query(
              ...
              return {
                  ...order,
                  items: itemsWithProductNames, // Using the more detailed items
                  simpleItems: simpleItems // Store the simple items separately if needed
              };
          })
      );
